...
class LaneNetPostProcessor(object):
    ...
    """
    def __init__(self, ipm_remap_file_path='ipm_remap.yml'):
        """

        :param ipm_remap_file_path: ipm generate file path
        """
        ipm_remap_file_path = ops.join(ops.dirname(ops.realpath(__file__)), ipm_remap_file_path)
        ...
        self._color_map = [np.array([255, 0, 0]),
                           ...
                           np.array([50, 100, 50]),
                           np.array([100, 50, 100])]
        # left lane colors, ordered in distance to ipm view center, up to 4 lines
        self._lcolor_map =[np.array([255, 0, 0]),
                           np.array([0, 125, 125]),
                           np.array([50, 100, 100]),
                           np.array([100, 50, 100])]
        # right lane colors, ordered in distance to ipm view center, up to 4 lines
        self._rcolor_map =[np.array([0, 0, 255]),
                           np.array([0, 255, 0]),
                           np.array([125, 125, 0]),
                           np.array([100, 100, 50])]
    ...
    def postprocess(self, binary_seg_result, instance_seg_result=None, min_area_threshold=100, source_image=None):
        ...
        # following is to facilitate fixed color pattern in returned source_image  -or-
        #                 agumented lane points mask image to improve after performance
        l_lanes = {}
        r_lanes = {}
        l_lane_pts = []  # ordered left lane points, based on distance to ipm view (640x640): bottom center [350, 640]
        r_lane_pts = []  # ordered right lane points, based on distance to ipm view (640x640): bottom center [350, 640]
        for lane_index, coords in enumerate(lane_coords):
            ...
            ploy_y_max = ipm_image_height
            fit_x_near = fit_param[0] * ploy_y_max ** 2 + fit_param[1] * ploy_y_max + fit_param[2]    # x at nearest

            lane_pts = []
            for index in range(0, plot_y.shape[0], 5):
                src_x = self._remap_to_ipm_x[int(plot_y[index]), int(np.clip(fit_x[index], 0, ipm_image_width - 1))]
                if src_x <= 0:
                    continue
                src_y = self._remap_to_ipm_y[int(plot_y[index]), int(np.clip(fit_x[index], 0, ipm_image_width - 1))]
            ...
            # base on ipm observation #
            tmp_ipm_x_ctr = 350   #####
            dist_to_x_ctr = fit_x_near - tmp_ipm_x_ctr
            if dist_to_x_ctr > 0:
                r_lanes[dist_to_x_ctr] = lane_pts
            else:
                l_lanes[dist_to_x_ctr] = lane_pts

        ### sort out right side lane lines in order of distance
        if len(r_lanes) > 0:
            inc_orders = sorted(r_lanes.keys())
            for ln in inc_orders:
                r_lane_pts.append(r_lanes[ln])

        ### sort out left side lane lines in order of distance
        if len(l_lanes) > 0:
            dec_orders = sorted(l_lanes.keys(), reverse=True)
            for ln in dec_orders:
                l_lane_pts.append(l_lanes[ln])

        l_lane_image = np.zeros_like(source_image, dtype=np.uint8)
        r_lane_image = np.zeros_like(source_image, dtype=np.uint8)

        # test data sample point along y axis every 10 pixels
        # for index, single_lane_pts in enumerate(src_lane_pts):

        ### plot ordered right lane lines in source_image and r_lane_image
        for index, single_lane_pts in enumerate(r_lane_pts):
            single_lane_pt_x = np.array(single_lane_pts, dtype=np.float32)[:, 0]
            single_lane_pt_y = np.array(single_lane_pts, dtype=np.float32)[:, 1]
            start_plot_y = 330    # Good : Carla Camera 600x800, bottom 270x800
            end_plot_y = rows
            ...
            for plot_y in np.linspace(start_plot_y, end_plot_y, step):
                ...
                interpolation_src_pt_x = (abs(previous_src_pt_y - plot_y) * previous_src_pt_x +
                                          abs(last_src_pt_y - plot_y) * last_src_pt_x) * 1.0 / \
                ...
                interpolation_src_pt_y = (abs(previous_src_pt_y - plot_y) * previous_src_pt_y +
                                          abs(last_src_pt_y - plot_y) * last_src_pt_y) * 1.0 / \
                                         (abs(previous_src_pt_y - plot_y) + abs(last_src_pt_y - plot_y))
                ...
                lane_color = self._rcolor_map[index].tolist()
                cv2.circle(source_image, (int(interpolation_src_pt_x),
                                          int(interpolation_src_pt_y)), 5, lane_color, -1)
                cv2.circle(r_lane_image, (int(interpolation_src_pt_x),
                                          int(interpolation_src_pt_y)), 5, lane_color, -1)

        ### plot ordered left lane lines in source_image and r_lane_image
        for index, single_lane_pts in enumerate(l_lane_pts):
            single_lane_pt_x = np.array(single_lane_pts, dtype=np.float32)[:, 0]
            single_lane_pt_y = np.array(single_lane_pts, dtype=np.float32)[:, 1]
            start_plot_y = 330    # Good : Carla Camera 600x800, bottom 270x800
            end_plot_y = rows
            ...
            for plot_y in np.linspace(start_plot_y, end_plot_y, step):
                ...
                #lane_color = self._color_map[index].tolist()
                lane_color = self._lcolor_map[index].tolist()
                cv2.circle(source_image, (int(interpolation_src_pt_x),
                                          int(interpolation_src_pt_y)), 5, lane_color, -1)
                cv2.circle(l_lane_image, (int(interpolation_src_pt_x),
                                          int(interpolation_src_pt_y)), 5, lane_color, -1)
        ...
        ret = {
            ...
            'l_lane_image': l_lane_image,
            'r_lane_image': r_lane_image
        }
